...
export class AxiosManager {
  ...
  constructor() {
    ...
    this.refreshTokenAxios.interceptors.request.use(this.refreshTokenRequestInterceptor);
  ...
  private async authRequestInterceptor(axiosConfig: InternalAxiosRequestConfig) {
  ...
  private async refreshTokenRequestInterceptor(axiosConfig: InternalAxiosRequestConfig) {
  ...
  private async refreshTokenRejectResponseInterceptor(error: AxiosError<{ errors?: string[] }>) {
    const errors = error.response?.data?.errors ?? [];
    const isDevBypass = useStore.getState().refreshToken === 'dev-refresh-token';
    if (
      error.response?.status === 401 &&
      (errors.includes('refresh token is invalid') || errors.includes('refresh token is expired')) &&
      !isDevBypass
    ) {
  ...
  private async rejectResponseInterceptor(error: AxiosError<{ errors?: string[] }>) {
    ...
    const isDevBypass = useStore.getState().accessToken === 'dev-access-token';
    if (
      error.response?.status === 401 &&
  ...
  private async refreshAccessToken() {
    const {
      data: { access_token },
    } = await this.refreshTokenAxios.post<{ access_token: string }>('auth/refresh');

    useStore.setState({ accessToken: access_token });
    this.axiosWithAuth.defaults.headers.common.Authorization = 'Bearer ' + access_token;
  }
}
